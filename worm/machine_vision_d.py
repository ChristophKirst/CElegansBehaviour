...
def create_normal(tav):
  #normal
  nrm = tf.mul(tav, tf.constant([-1,1], "float32"));
...
def create_peristaltic(c, tav, p):
  return tf.add(c, tf.mul(p, tav))
  
def create_bend(c, n, b, p):
  pb = tf.reduce_sum(tf.transpose(tf.mul(tf.transpose(p), b)), reduction_indices = 0);
  s = tf.transpose(tf.mul(pb, tf.transpose(n)));
  return tf.add(c, s);
