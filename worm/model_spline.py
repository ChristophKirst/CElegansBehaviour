...
The worm is parameterized by:
  ...
  * b     - coefficients for the b spline interpolation of the bending of the center line
  * w     - cefficients for the b spline interpolation of the width profile along that center line
  * l     - length of the center line
  * xy    - position of the worm (the center of the mid line)
  * theta - absolute orientation of the worm

Additional internal parameters that define the model are:
  * degree  - the degree of the b splines
  * nsample - sample points along the body outline
...
class WormModel:
  ...
  def __init__(self, l = 40, xy = [75, 75], theta = 0, b = None, w = None, nbasis = 10, degree = 3, nsamples = 21, valid = True):
    """Constructor of WormModel
    
    Arguments:
      l (number): length of the center line of the worm [in pixel]
      xy (2 array): position of the center of the center line [in pixel]
      theta (number): absolute orientation of the worm
      w (nbasis array or None): bspline coefficients of the width profile of the worm (if none use default profile)
      b (nbasis array or None): bspline coefficients of the center bending angle of the worm center line (if None strait worm)
      nbasis (int or None): number of reference points if b and w is None
      degree (int): degree of the bspline
      nsamples (int): number of sample points along the worms side lines
    """
    ...
    if nbasis is None:
      if w is not None:
        ...
        raise RuntimeError('cannot determine number of basis points for spline interpolation!');
    else:
      self.nbasis = nbasis;
    ...
    if nsamples % 2 != 1:
      raise RuntimeWarning('WormModel: number of sample points expected to be odd, adding a point!')
      nsamples += 1;
    self.nsamples = int(nsamples); # number of segments
    
    
    # set up the projection matrices etc
    self.initialize_splines();
    
    if b is None:
      self.b = np.zeros(nbasis, dtype = float);
    else:
      self.b = np.array(b, dtype = float);
    
    if w is None:
      self.w = self.width_to_w(self.guess_width())
    else:
