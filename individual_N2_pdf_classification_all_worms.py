...
#open the memmaps for the distributions etc...

##############################################################################
#joint distributions for each stage

stages = np.unique(w.stage());

joint_data = [None for s in stages];

delays = [30, 100, 200, 400];
delays_max = max(delays)+1
delays_n2 = int(np.ceil(len(delays)/2.))

more_delays = [0, 5, 30, 100, 200, 400];
more_delays_n2 = int(np.ceil(len(more_delays)/2.))
...
for s in stages:
  ...
  fig, axs = plt.subplots(2, more_delays_n2);
  for i,d in enumerate(more_delays):
...
dists= w.calculateDistances(n = delays_max);
rots  = w.calculateRotations(n = delays_max);
...
#save values for cumulative distribution
