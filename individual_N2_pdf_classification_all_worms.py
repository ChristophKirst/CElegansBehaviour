...
ndelays = len(delays);
n2delays = int(np.ceil(ndelays/2.))

more_delays = [0, 5, 30, 100, 200, 400];
more_delays_n2 = int(np.ceil(len(more_delays)/2.))
n_more_delays = len(more_delays);
n2_more_delays = int(np.ceil(n_more_delays/2.))
...
for s in stages:
  for f in features:
    fn = os.path.join(datadir, 'data_stage%d_%s.png' % (s,f))  
    np.lib.format.open_memmap(fn, shape = (ssize[s-1], ndelays), mode = 'w+');
...
for s in stages:
  for wid in range(nworms):
    ...
    fig, axs = plt.subplots(2, n2_more_delays);
